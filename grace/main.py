...
def run(config: DictConfig):
    ...
    batch_history = []
    log_list = []
    for i, batch in tqdm(enumerate(edit_loader),total=len(edit_loader)):
        ...
        if is_error(editor, tokens) & (n_edits <= config.max_n_edits):
            ...
            log_dict["TRR"] = TRR.item() # Test Retention Rate
            log_dict["ERR"] = ERR.item() # Error Retention Rate
            log_dict["ES"] =  ES.item() # Edit Success
            log_dict["train_time"] = edit_time/60 # Time it takes to make one edit
            log_dict["edit"] = batch["text"] # Raw edit input
            log_dict["edit_label"] = batch["labels"].item() # Raw edit label
    ...
    with open(
        f'./results_{config.experiment.task}_{config.editor._name}_eps{config.editor.eps}.json','w') as f:
